...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      'next/navigation': path.resolve('./src/__tests__/mocks/next-navigation.ts'),
    },
  },
});
```
...
module.exports = {
  ...
  transform: {
    '^.+\\.(t|j)s$': ['ts-jest', {        // 用 ts-jest 直接读 TypeScript 源文件
  ...
  moduleNameMapper: {
    ...
    '^@api/(.*)$': '<rootDir>/$1',
    '^@myorg/shared$': '<rootDir>/../../../node_modules/@myorg/shared/dist/index.js',
  },
};
```
...
describe('MyFeatureService', () => {
  ...
  describe('findOne(id)', () => {
    it('记录存在时返回数据', async () => {
      ...
      // 验证调用参数正确（不只是"被调用了"）
      expect(mockPrisma.myModel.findUnique).toHaveBeenCalledWith({ where: { id: '1' } });
    });
  ...
  describe('create(dto)', () => {
    ...
    it('成功创建并返回新记录', async () => {
      ...
      mockPrisma.myModel.create.mockResolvedValue({ id: 'new-id', name: 'new' });
...
it.each(INVALID_DOMAINS)('拒绝垃圾邮箱域名: %s', async (domain) => {
  await expect(
    service.create({ email: `user@${domain}` }),
...
export async function expectNoError(page: Page) {
  await expect(page.locator('body'))
    .not.toContainText('Application error', { timeout: 10_000 });
}
...
export async function loginViaUI(page: Page) {
  ...
  await page.getByLabel('Password').fill(process.env.E2E_ADMIN_PASSWORD ?? 'Admin@123456');
...
test.describe('My Page — /my-page', () => {
  ...
  test('页面加载不崩溃', async ({ page }) => {
    await expect(page.locator('body'))
      .not.toContainText('Application error', { timeout: 20_000 });
  });
...
test.describe('Xxx Page — /xxx', () => {
  ...
  test('空提交触发校验错误', async ({ page }) => {
    ...
    await expect(
      page.locator('[class*="text-red"], [class*="error"], [role="alert"]').first(),
    ).toBeVisible({ timeout: 8_000 });
  });
});
```
