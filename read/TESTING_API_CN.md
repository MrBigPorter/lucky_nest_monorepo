...
await waitFor(() => {
  expect(mockAuthLogin).toHaveBeenCalledWith({ username: 'admin', password: 'password123' });
...
// 使用
mockGet('http://localhost/api/users', { code: 10000, message: 'ok', data: [{ id: 1 }] });
```
...
// 链式过滤
page.locator('h1').filter({ hasText: /dashboard/i }).first()
...
**签名**：
```typescript
function makeUseAntdTable<T>(list: T[], total = list.length, loading = false): UseAntdTableReturn<T>
```
