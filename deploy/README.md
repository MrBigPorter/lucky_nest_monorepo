...
| Secret | 用途 | 示例 |
|--------|------|------|
| `SSH_HOST` | VPS IP | `107.175.53.104` |
...
# 将公钥添加到 VPS
ssh-copy-id -i ~/.ssh/github_deploy_key.pub root@107.175.53.104
...
# 手动备份
ssh root@107.175.53.104 '/opt/lucky/deploy/backup.sh'

# 下载备份到本地
scp root@107.175.53.104:/opt/lucky/backups/backup_latest.dump.gz ./
```
...
# 查看监控日志
ssh root@107.175.53.104 'tail -50 /var/log/lucky-monitor.log'

# 查看告警
ssh root@107.175.53.104 'tail -20 /var/log/lucky-alerts.log'
```
...
# 查看续期日志
ssh root@107.175.53.104 'tail -20 /var/log/lucky-cert.log'
```
...
```bash
# 查看日志
ssh root@107.175.53.104 'cd /opt/lucky && docker compose -f compose.prod.yml logs -f --tail=100'

# 查看资源使用
ssh root@107.175.53.104 'docker stats --no-stream'

# 重启某个服务
ssh root@107.175.53.104 'cd /opt/lucky && docker compose -f compose.prod.yml restart backend'

# 进入容器调试
ssh root@107.175.53.104 'docker exec -it lucky-backend-prod sh'

# 执行数据库迁移
ssh root@107.175.53.104 'docker exec lucky-backend-prod ./apps/api/node_modules/.bin/prisma migrate deploy --schema=apps/api/prisma/schema.prisma'

# 清理 Docker 垃圾
ssh root@107.175.53.104 'docker system prune -af --filter "until=72h"'

# 查看 swap 使用
ssh root@107.175.53.104 'free -h && swapon --show'
```
