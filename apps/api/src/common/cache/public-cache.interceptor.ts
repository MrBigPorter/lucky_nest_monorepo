...
function normalizeHeader(v: unknown, fallback: string) {
    return String(v ?? fallback).toLowerCase().replace(/[^a-z0-9_-]/g, '');
}
...
export class PublicCacheInterceptor extends CacheInterceptor {
    trackBy(context: ExecutionContext): string | undefined {
        ...
        // 调试绕过（query 或 header）
        if (req?.query?.__nocache === '1' || req?.headers?.['x-debug-nocache'] === '1') {
            return undefined;
        }
        ...
        const path: string =
            req.path || req.originalUrl?.split('?')[0] || req.url?.split('?')[0] || '/';
