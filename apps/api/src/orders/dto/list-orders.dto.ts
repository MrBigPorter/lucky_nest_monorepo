...
export class ListOrdersDto {
    @ApiPropertyOptional({ description: 'status', example: 'all', enum: ['all', 'paid', 'unpaid', 'refunded', 'cancelled']})
    @IsOptional()
    @IsString()
    ...
    @ApiPropertyOptional({ description: 'treasureId', example: 'uuid-v4', type: String})
    @IsOptional()
    @IsString()
    treasureId?: string
}