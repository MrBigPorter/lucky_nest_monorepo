...
const COUPONS = [
  // ① 新用户欢迎券：满₱200减₱50
  {
    ...
    description:   'Welcome gift for new JoyMinis users. Valid for 7 days after claiming.',
    couponType:    1,    // 满减
    discountType:  1,    // 固定金额
    discountValue: 50,
    minPurchase:   200,
  ...
  // ③ 无门槛现金券：直减₱30
  {
    ...
    description:   'No strings attached! ₱30 off on any product. Valid 7 days after claiming.',
    couponType:    3,    // 无门槛现金
    discountType:  1,
    discountValue: 30,
    minPurchase:   0,
  ...
  // ④ 大额满减：满₱1,000减₱200
  {
    ...
    description:   'For big spenders! ₱200 off orders of ₱1,000 or more. Limited 100 coupons.',
    couponType:    1,
    discountType:  1,
    discountValue: 200,
    minPurchase:   1000,
...
export async function seedCoupons() {
  ...
  for (const { maxDiscount, ...row } of COUPONS) {
    const e = await db.coupon.findUnique({ where: { couponCode: row.couponCode } });
    if (!e) {
