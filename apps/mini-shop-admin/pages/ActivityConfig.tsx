...
export const ActivityConfig: React.FC = () => {
  const {
    ...
    remove,
    update,
  } = useMockData<LotteryActivity>(MOCK_LOTTERY_ACTIVITIES);
  const [view, setView] = useState<'list' | 'builder'>('list');
  const [currentActivity, setCurrentActivity] = useState<
    Partial<LotteryActivity>
  >({});
  const [activeStep, setActiveStep] = useState(1);

  // DnD States
  const [draggedActivityIndex, setDraggedActivityIndex] = useState<
    number | null
  >(null);
  const [draggedPrizeIndex, setDraggedPrizeIndex] = useState<number | null>(
    null,
  );
  ...
  const removePrize = (index: number) => {
    const newPrizes = (currentActivity.prizes || []).filter(
      (_, i) => i !== index,
    );
    setCurrentActivity({ ...currentActivity, prizes: newPrizes });
  };
  ...
  const addPrize = () => {
    ...
    setCurrentActivity({
      ...currentActivity,
      prizes: [...(currentActivity.prizes || []), newPrize],
    });
  };
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
              Create and manage lucky draws, wheels, and mystery boxes
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedActivities.map((act, index) => (
            ...
            >
              <Card
                className={`group relative overflow-hidden hover:shadow-lg transition-all border-l-4 border-l-primary-500 cursor-move ${draggedActivityIndex === index ? 'opacity-50' : ''}`}
              >
        ...
        <div className="flex items-center">
          {[
            ...
            <div key={step.id} className="flex items-center">
              ...
              {idx < 2 && (
                <div className="w-12 h-px bg-gray-200 dark:bg-white/10 mx-2" />
              )}
            </div>
          ))}
        </div>
            ...
            <Card title="Activity Details">
              <div className="space-y-4">
                ...
                <Input
                  ...
                  value={currentActivity.coverImage}
                  onChange={(e) =>
                    setCurrentActivity({
                      ...currentActivity,
          ...
          <div className="max-w-4xl mx-auto">
            ...
            >
              <div className="space-y-4">
                    ...
                    <div className="w-full md:w-auto flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 pl-6">
                      <div className="md:col-span-1">
                        ...
                        <Select
                          value={prize.type}
                          onChange={(e) =>
                            updatePrize(idx, 'type', e.target.value)
                          }
                          options={[
                      ...
                      <div className="md:col-span-2">
                        ...
                        {prize.type === 'product' ? (
                          ...
                          <Input
                            placeholder={
                              prize.type === 'empty' ? 'Message' : 'Amount'
                            }
                            value={
                              prize.type === 'empty'
                                ? prize.name
                            ...
                            onChange={(e) => {
                              if (prize.type === 'empty')
                                updatePrize(idx, 'name', e.target.value);
                      ...
                      <div className="md:col-span-1">
                        ...
                        <Input
                          value={prize.image}
                          onChange={(e) =>
                            updatePrize(idx, 'image', e.target.value)
                          }
                          placeholder="http://..."
                        />
                      </div>
                    ...
                    <div className="w-full md:w-48 grid grid-cols-2 gap-4 border-l border-gray-200 dark:border-white/10 pl-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={prize.stock}
                          onChange={(e) =>
                            updatePrize(idx, 'stock', Number(e.target.value))
                          }
                        />
                      </div>
                    </div>
                ...
                {(!currentActivity.prizes ||
                  currentActivity.prizes.length === 0) && (
                ...
                <div className="flex justify-end text-sm text-gray-500">
                  Total Probability:{' '}
                  <span
                    className={`font-bold ml-1 ${(currentActivity.prizes?.reduce((acc, p) => acc + p.probability, 0) || 0) === 100 ? 'text-green-500' : 'text-red-500'}`}
                  >
                    {currentActivity.prizes?.reduce(
                      (acc, p) => acc + p.probability,
                      0,
                    ) || 0}
                    %
                  </span>
                      ...
                      <div
                        ...
                        onClick={() =>
                          setCurrentActivity({
                            ...currentActivity,
                            rules: {
                              ...currentActivity.rules!,
                              costType: type as any,
                            },
                          })
                        }
