...
export const VipConfig: React.FC = () => {
  ...
  const addBenefit = () => {
    if (!editingTier) return;
    setEditingTier({ ...editingTier, benefits: [...editingTier.benefits, 'New Benefit'] });
  };

  const removeBenefit = (index: number) => {
    if (!editingTier) return;
    setEditingTier({ ...editingTier, benefits: editingTier.benefits.filter((_, i) => i !== index) });
  };

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">VIP System Configuration</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Define thresholds and privileges for user loyalty tiers</p>
      </div>
      ...
      <div className="flex overflow-x-auto pb-6 gap-6 custom-scrollbar snap-x">
        {tiers.map(tier => (
           <div key={tier.level} className="min-w-[300px] snap-center">
              <div className={`h-full rounded-2xl p-6 relative overflow-hidden flex flex-col ${tier.color} bg-opacity-20 dark:bg-opacity-10 border-2 border-transparent hover:border-primary-500/50 transition-all group`}>
                 <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Crown size={100} />
                 </div>
                 
                 <div className="relative z-10 flex justify-between items-start mb-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg ${tier.color.replace('bg-', 'bg-').replace('10', '500')}`}>
                       <span className="font-bold text-xl">{tier.level}</span>
                    </div>
                    <button onClick={() => setEditingTier(tier)} className="p-2 bg-white/20 hover:bg-white/40 rounded-lg text-gray-700 dark:text-white transition-colors">
                       <Edit2 size={16} />
                    </button>
                 </div>

                 <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{tier.name}</h3>
                 <p className="text-sm font-medium opacity-60 mb-6">Threshold: ${tier.threshold.toLocaleString()}</p>
      ...
      <Modal isOpen={!!editingTier} onClose={() => setEditingTier(null)} title={`Edit VIP ${editingTier?.level} - ${editingTier?.name}`}>
         {editingTier && (
                  ...
                  <div className="space-y-2">
                     {editingTier.benefits.map((benefit, index) => (
                        <div key={index} className="flex gap-2">
                           ...
                           />
                           <button onClick={() => removeBenefit(index)} className="text-red-500 hover:text-red-600 px-2">
                              &times;
                           </button>
                        </div>
                     ))}
                     <Button size="sm" variant="secondary" onClick={addBenefit} className="w-full mt-2">
