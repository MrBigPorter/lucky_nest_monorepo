...
import { Play, RotateCw, History, Hash, CheckCircle, Clock } from 'lucide-react';
...
export const LotteryControl: React.FC = () => {
  ...
  const handleDraw = (id: string) => {
    ...
    let interval = setInterval(() => {
      setSimulatedHash(Array.from({length: 64}, () => Math.floor(Math.random()*16).toString(16)).join(''));
    }, 50);
    ...
    setTimeout(() => {
      ...
      const mockWinner = { id: '999', name: 'LuckyUser', code: Math.floor(10000 + Math.random() * 90000).toString() };
      const finalHash = Array.from({length: 64}, () => Math.floor(Math.random()*16).toString(16)).join('');
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Lottery Control Center</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Monitor fill rates and execute fairness algorithms</p>
      </div>
      ...
      {calculatingId && (
         <Card className="bg-gradient-to-r from-gray-900 to-black text-white border-primary-500/50 border animate-pulse">
            <div className="flex flex-col items-center py-8">
               <RotateCw className="animate-spin text-primary-500 mb-4" size={48} />
               ...
               <p className="text-gray-400 text-sm mt-4">Validating blockchain timestamp...</p>
            </div>
         </Card>
      )}
         ...
         {pendingDraws.map(draw => (
            <Card key={draw.id} className="relative overflow-hidden">
               <div className="flex gap-4">
                  <img src={draw.product.image} className="w-24 h-24 rounded-lg object-cover bg-gray-100 dark:bg-white/5" />
                  <div className="flex-1">
                     <div className="flex justify-between items-start">
                        <div>
                           <h3 className="font-bold text-lg text-gray-900 dark:text-white">{draw.product.name}</h3>
                           <p className="text-xs text-gray-500">Draw ID: {draw.id}</p>
                        </div>
                        <Badge color={draw.fillRate >= 100 ? 'green' : 'yellow'}>
                           {draw.fillRate >= 100 ? 'Ready' : `${draw.fillRate}% Filled`}
                        </Badge>
                     </div>
               ...
               <div className="mt-6 flex justify-end">
                  <Button 
                     ...
                     className={draw.fillRate < 100 ? 'opacity-50 cursor-not-allowed' : ''}
                  >
            ...
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {historyDraws.map(draw => (
                     <tr key={draw.id} className="group hover:bg-gray-50 dark:hover:bg-white/5">
                        <td className="py-4 pl-4">
                           <div className="flex items-center gap-3">
                              <img src={draw.product.image} className="w-8 h-8 rounded" />
                              <span className="font-medium text-gray-900 dark:text-white">{draw.product.name}</span>
                           </div>
                        </td>
                        <td className="py-4 text-sm font-bold text-primary-500">{draw.winner?.name}</td>
                        <td className="py-4 font-mono text-sm bg-gray-100 dark:bg-white/10 px-2 rounded w-fit">{draw.winner?.code}</td>
                        <td className="py-4 font-mono text-xs text-gray-500">{draw.hash?.substring(0, 16)}...</td>
                        <td className="py-4 text-xs text-gray-500">{draw.drawTime}</td>
