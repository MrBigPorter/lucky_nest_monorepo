...
import { Wallet, PlusCircle, Edit, Trash2, TrendingUp, TrendingDown, CheckCircle, XCircle, FileText, AlertTriangle, ShieldCheck, ArrowRightLeft, Search } from 'lucide-react';
import { Card, Button, Input, Modal, Badge, DateRangePicker } from '../components/UIComponents';
import { MOCK_RECHARGE_PLANS, MOCK_WITHDRAWALS, MOCK_RECHARGE_ORDERS, MOCK_TRANSACTIONS } from '../constants';
...
const RechargeConfig: React.FC = () => {
  const { data: plans, add, remove, update } = useMockData<RechargePlan>(MOCK_RECHARGE_PLANS);
  const toast = useToast();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<Partial<RechargePlan> | null>(null);
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {plans.map(plan => (
          <div key={plan.id} className="group bg-white dark:bg-dark-800 rounded-2xl border border-gray-100 dark:border-white/5 p-6 flex flex-col items-center text-center relative hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-primary-500/10 hover:border-primary-500/30">
            {plan.tag && (
            ...
            <h3 className="text-4xl font-bold text-gray-900 dark:text-white mb-2 tracking-tight">${plan.amount}</h3>
            ...
            <div className="grid grid-cols-2 gap-3 w-full mt-auto opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0">
               <Button variant="secondary" size="sm" onClick={() => handleOpenModal(plan)} className="w-full">
                 <Edit size={14} /> Edit
               </Button>
               <Button variant="danger" size="sm" onClick={() => remove(plan.id)} className="w-full bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white shadow-none">
                 <Trash2 size={14} />
               </Button>
            </div>
          </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Recharge Plan' : 'Create Recharge Plan'} size="sm">
                   ...
                   <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                      {filteredDeposits.map(d => (
                         <tr key={d.id} className="group hover:bg-gray-50 dark:hover:bg-white/5">
                            <td className="py-4 pl-4 font-mono text-sm text-gray-500">{d.orderNo}</td>
                            <td className="py-4">
                               <div className="flex items-center gap-3">
                                  <img src={d.user.avatar} className="w-8 h-8 rounded-full" />
                                  <span className="font-medium text-gray-900 dark:text-white">{d.user.name}</span>
                               </div>
                            </td>
                            <td className="py-4 font-bold text-gray-900 dark:text-white">₱{d.amount.toLocaleString()}</td>
                            ...
                            <td className="py-4 text-right pr-6 text-sm text-gray-500">{d.date}</td>
                         </tr>
                      ))}
                      {filteredDeposits.length === 0 && (
                          <tr>
                              <td colSpan={7} className="text-center py-8 text-gray-400">No deposit records found matching your search.</td>
                          </tr>
                      )}
                   </tbody>
                </table>
...
const TransactionLogs: React.FC = () => {
    ...
    const getTypeColor = (type: string) => {
        switch(type) {
            ...
            case 'win': return 'text-amber-500';
        ...
        <Card title="Transaction History" action={<DateRangePicker />}>
             <div className="overflow-x-auto">
                <table className="w-full text-left">
                   ...
                   <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                      {logs.map(log => (
                         <tr key={log.id} className="group hover:bg-gray-50 dark:hover:bg-white/5">
                            <td className="py-4 pl-4 font-mono text-xs text-gray-500">{log.transactionNo}</td>
                            <td className="py-4 text-sm font-medium text-gray-900 dark:text-white">{log.user.name}</td>
                            <td className="py-4">
                               <span className={`capitalize text-sm font-bold ${getTypeColor(log.type)}`}>
                                   {log.type}
                               </span>
                            </td>
                            ...
                            <td className="py-4 text-xs text-gray-500">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-gray-900 dark:text-white">{log.balanceAfter.toLocaleString()}</span>
                                </div>
                            </td>
                            <td className="py-4 text-right pr-6 text-sm text-gray-500">{log.date}</td>
                         </tr>
                      ))}
                   </tbody>
                </table>
...
const WithdrawalAudit: React.FC = () => {
  const { data: withdrawals, update } = useMockData<Withdrawal>(MOCK_WITHDRAWALS);
  const toast = useToast();
  const [riskModal, setRiskModal] = useState<Withdrawal | null>(null);

  const handleStatus = (id: string, status: 'approved' | 'rejected') => {
    update(id, { status });
    toast.addToast(status === 'approved' ? 'success' : 'error', `Withdrawal ${status.toUpperCase()}`);
    setRiskModal(null);
  };
  ...
  return (
    <>
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                {withdrawals.map(w => (
                <tr key={w.id} className="group hover:bg-gray-50 dark:hover:bg-white/5">
                    <td className="py-4 pl-4">
                    <div className="flex items-center gap-3">
                        <img src={w.user.avatar} className="w-8 h-8 rounded-full" />
                        <span className="font-medium text-gray-900 dark:text-white">{w.user.name}</span>
                    </div>
                    </td>
                    <td className="py-4 font-mono font-medium">₱{w.amount.toLocaleString()}</td>
                    ...
                    {w.status === 'pending' && (
                        <Button size="sm" onClick={() => setRiskModal(w)} className="bg-primary-500/10 text-primary-600 hover:bg-primary-500/20 shadow-none border-0">
        ...
        <Modal isOpen={!!riskModal} onClose={() => setRiskModal(null)} title="Risk Control Audit">
            {riskModal && (
                <div className="space-y-6">
                    <div className="bg-gray-50 dark:bg-white/5 p-4 rounded-xl flex items-center justify-between">
                         <div className="flex items-center gap-4">
                             <img src={riskModal.user.avatar} className="w-16 h-16 rounded-full" />
                             <div>
                                 <h3 className="text-xl font-bold text-gray-900 dark:text-white">{riskModal.user.name}</h3>
                                 <p className="text-sm text-gray-500">ID: {riskModal.user.id}</p>
                             </div>
                         </div>
                         <div className="text-right">
                             <div className="text-sm text-gray-500">Requested Amount</div>
                             <div className="text-2xl font-bold text-gray-900 dark:text-white">₱{riskModal.amount.toLocaleString()}</div>
                         </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl border border-gray-100 dark:border-white/10 bg-green-50 dark:bg-green-900/10">
                             <div className="text-xs text-gray-500 uppercase">Total Deposit</div>
                        ...
                        <div className="p-4 rounded-xl border border-gray-100 dark:border-white/10 bg-red-50 dark:bg-red-900/10">
                             <div className="text-xs text-gray-500 uppercase">Total Withdraw</div>
                        ...
                        <div className="p-4 rounded-xl border border-gray-100 dark:border-white/10 bg-blue-50 dark:bg-blue-900/10 col-span-2">
                             <div className="flex justify-between items-center mb-1">
                                <div className="text-xs text-gray-500 uppercase">Wager Requirement</div>
                                <div className="text-xs font-bold text-blue-600">85% Completed</div>
                             </div>
                    ...
                    <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-500/20 p-4 rounded-xl flex items-start gap-3">
                         ...
                         <div>
                             <h4 className="font-bold text-yellow-800 dark:text-yellow-500 text-sm">Risk Alert</h4>
                             <p className="text-xs text-yellow-700 dark:text-yellow-400 mt-1">
                                User IP address changed frequently in the last 24 hours. Multiple accounts detected on same device ID.
                             </p>
                         </div>
                    </div>

                    <div className="flex gap-4 pt-2">
                        <Button variant="danger" className="flex-1" onClick={() => handleStatus(riskModal.id, 'rejected')}>
                            <XCircle size={18} /> Reject
                        </Button>
                        <Button variant="primary" className="flex-1 bg-green-600 hover:bg-green-700 border-green-600" onClick={() => handleStatus(riskModal.id, 'approved')}>
...
export const Finance: React.FC = () => {
  const [tab, setTab] = useState<'overview' | 'deposits' | 'withdrawals' | 'transactions' | 'config'>('overview');
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 dark:from-black dark:to-dark-900 rounded-2xl p-8 text-white relative overflow-hidden shadow-2xl border border-gray-800 dark:border-white/10">
         ...
         <div className="relative z-10">
            ...
            <p className="text-gray-400 max-w-xl">Comprehensive financial monitoring, audit, and risk control.</p>
         </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-gray-200 dark:border-white/10 pb-1">
           {[
               ...
               {id: 'deposits', label: 'Deposit Orders', icon: <ArrowRightLeft size={16}/>},
               {id: 'withdrawals', label: 'Withdrawal Audit', icon: <ShieldCheck size={16}/>},
               {id: 'transactions', label: 'Transaction Logs', icon: <FileText size={16}/>},
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="bg-emerald-500/10 border-emerald-500/20">
                      <div className="flex justify-between items-start">
                          <div>
                              <p className="text-emerald-600 dark:text-emerald-400 font-bold text-sm uppercase">Total Deposit Today</p>
                              <h3 className="text-3xl font-bold text-gray-900 dark:text-white mt-2">₱124,500</h3>
                          </div>
                  ...
                  <Card className="bg-red-500/10 border-red-500/20">
                      <div className="flex justify-between items-start">
                          <div>
                              <p className="text-red-600 dark:text-red-400 font-bold text-sm uppercase">Total Payout Today</p>
                              <h3 className="text-3xl font-bold text-gray-900 dark:text-white mt-2">₱42,300</h3>
                          </div>
                  ...
                  <Card className="bg-blue-500/10 border-blue-500/20">
                      <div className="flex justify-between items-start">
                          <div>
                              <p className="text-blue-600 dark:text-blue-400 font-bold text-sm uppercase">Net Profit</p>
                              <h3 className="text-3xl font-bold text-gray-900 dark:text-white mt-2">₱82,200</h3>
                          </div>
