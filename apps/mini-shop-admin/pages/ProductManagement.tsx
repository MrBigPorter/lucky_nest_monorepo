...
import { Plus, Edit2, Trash2, Search, Filter, DollarSign, AlertTriangle, TrendingUp, TrendingDown, Tag, RotateCcw, GripVertical } from 'lucide-react';
import { Card, Button, Input, Modal, Badge, Select, Switch, ImageUpload, DragHandle, Textarea } from '../components/UIComponents';
import { MOCK_PRODUCTS, MOCK_CATEGORIES, TRANSLATIONS } from '../constants';
...
export const ProductManagement: React.FC = () => {
  ...
  const { data: products, loading, add, remove, update } = useMockData<Product>(MOCK_PRODUCTS);
  ...
  const defaultProduct: Partial<Product> = { 
    name: '', price: 0, cost: 0, category: 'Electronics', totalShares: 100, soldShares: 0, 
    status: 'draft', image: '', lotteryMode: 1, sortOrder: 0, description: '', purchaseLimit: 0, tags: [], autoRestart: false
  };
  ...
  const handleDeleteClick = (product: Product) => {
      if (product.status === 'active') {
          toast.addToast('error', 'Action Blocked: Cannot delete an active product. Please deactivate it first.');
          return;
      }
      if (product.soldShares > 0) {
          toast.addToast('error', `Action Blocked: This product has sold ${product.soldShares} shares. Archive it instead.`);
          return;
      }
  ...
  const filteredProducts = products.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)).filter(p => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
           ...
           <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage treasure hunt items & inventory</p>
        </div>
        <Button onClick={() => handleOpenModal()} className="shadow-xl shadow-primary-500/20"><Plus size={18} /> {t.add}</Button>
      </div>
                  ...
                  <td className="py-4 pl-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 font-mono">{product.seq}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4">
                    <div className="flex flex-col w-32">
                      <div className="flex justify-between text-xs mb-1 text-gray-500">
                        <span>{Math.round((product.soldShares / product.totalShares) * 100)}%</span>
                        <span>{product.soldShares}/{product.totalShares}</span>
                      </div>
                      <div className="h-1.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-primary-500 rounded-full transition-all duration-500" style={{ width: `${(product.soldShares / product.totalShares) * 100}%` }}></div>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 font-mono text-gray-700 dark:text-gray-300 font-medium">₱{product.price}</td>
                  <td className="py-4">
                    <Badge color={product.status === 'active' ? 'green' : product.status === 'ended' ? 'gray' : 'yellow'}>
                      {product.status.toUpperCase()}
                    </Badge>
                  </td>
                  <td className="py-4 text-right pr-6">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => handleOpenModal(product)} className="p-2 hover:bg-primary-500/10 text-gray-400 hover:text-primary-500 rounded-lg transition-colors"><Edit2 size={16} /></button>
                      <button onClick={() => handleDeleteClick(product)} className="p-2 hover:bg-red-500/10 text-gray-400 hover:text-red-500 rounded-lg transition-colors"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* CREATE/EDIT MODAL */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Product' : 'New Product'} size="lg">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className={`rounded-xl p-4 border flex flex-col justify-center transition-colors ${profit >= 0 ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/20' : 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/20'}`}>
                    <div className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">Estimated Profit</div>
                    <div className="flex items-center gap-2">
                        <span className={`text-2xl font-bold ${profit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                            {profit >= 0 ? '+' : ''}₱{profit.toLocaleString()}
                        </span>
                        {profit >= 0 ? <TrendingUp size={20} className="text-emerald-500"/> : <TrendingDown size={20} className="text-red-500"/>}
                    </div>
                    <div className="text-sm mt-2 flex justify-between text-gray-600 dark:text-gray-300">
                        <span>Margin: <span className="font-bold">{margin.toFixed(1)}%</span></span>
          ...
          <Select
            ...
            onChange={e => setFormData({...formData, lotteryMode: Number(e.target.value) as 1 | 2})}
            options={[{label: 'Sold Out Mode (Auto)', value: 1}, {label: 'Timed Mode (Fixed)', value: 2}]}
          />
          ...
          {formData.lotteryMode === 2 && (
             <Input 
              ...
              onChange={e => setFormData({...formData, lotteryTime: e.target.value})} 
            />
          )}

          <div className="md:col-span-2 space-y-4 pt-4 border-t border-gray-100 dark:border-white/5">
              <h4 className="font-bold text-gray-900 dark:text-white">Content & Risk Control</h4>
              <Textarea 
              ...
              <div className="grid grid-cols-2 gap-4">
                  <Input 
                    ...
                    onChange={e => setFormData({...formData, purchaseLimit: Number(e.target.value)})}
                  />
                  <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Marketing Tags</label>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
                 <div className="flex items-center gap-2">
                     ...
                     <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Auto Restart Next Round</span>
                 </div>
                 <Switch checked={!!formData.autoRestart} onChange={c => setFormData({...formData, autoRestart: c})} />
              </div>
          </div>
          ...
          <div className="md:col-span-2 flex items-center justify-between p-4 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5 h-[74px] mt-2">
             ...
             <div className="flex items-center gap-3">
               <span className={`text-xs ${formData.status === 'active' ? 'text-primary-500 font-bold' : 'text-gray-400'}`}>
                 {formData.status?.toUpperCase()}
               </span>
               <Switch 
                 checked={formData.status === 'active'}
                 onChange={(checked) => setFormData({...formData, status: checked ? 'active' : 'draft'})}
               />
             </div>
          </div>
        </div>
      ...
      <Modal isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} title="Confirm Deletion" size="sm">
         <div className="space-y-4">
             <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl flex items-start gap-3 border border-red-100 dark:border-red-900/30">
                 ...
                 <div>
                     <h4 className="font-bold text-red-900 dark:text-red-400">Danger Zone</h4>
                     <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                         You are about to delete <strong>{deleteItem?.name}</strong>. This action is irreversible.
                     </p>
                 </div>
             </div>
...
export const CategoryManagement: React.FC = () => {
   const { data: categories, loading, add, remove, update } = useMockData<Category>(MOCK_CATEGORIES);
   const [isModalOpen, setIsModalOpen] = useState(false);
   const [editingItem, setEditingItem] = useState<Partial<Category> | null>(null);
   const [formData, setFormData] = useState<Partial<Category>>({ name: '', productCount: 0 });
     ...
     <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Organize your products structure</p>
          </div>
          <Button onClick={() => handleOpenModal()} variant="outline"><Plus size={18} /> Add Category</Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map(cat => (
            <Card key={cat.id} className="hover:border-primary-500/50 hover:shadow-lg hover:shadow-primary-500/5 cursor-pointer group relative">
               <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); handleOpenModal(cat); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-white/10 rounded text-gray-400 hover:text-primary-500"><Edit2 size={14}/></button>
                    <button onClick={(e) => { e.stopPropagation(); remove(cat.id); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-white/10 rounded text-gray-400 hover:text-red-500"><Trash2 size={14}/></button>
                  </div>
               </div>
               <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{cat.name}</h3>
               <p className="text-sm text-gray-500">{cat.productCount} Products linked</p>
            </Card>
          ))}
           <button 
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Category' : 'New Category'} size="sm">
           <div className="space-y-4">
              <Input 
                label="Category Name" 
                value={formData.name} 
