...
import { Shield, UserCog, FileText, Plus, Edit2, Trash2, CheckSquare, Square, Key } from 'lucide-react';
import { Card, Button, Input, Modal, Badge, Select } from '../components/UIComponents';
import { MOCK_ADMIN_USERS, MOCK_ROLES, MOCK_OPERATION_LOGS } from '../constants';
import { useMockData } from '../hooks/useMockData';
import { AdminUser, Role, OperationLog } from '../types';

// --- SUB-COMPONENT: ADMIN USERS ---
const AdminUsers: React.FC = () => {
    const { data: users, add, update, remove } = useMockData<AdminUser>(MOCK_ADMIN_USERS);
    ...
    return (
        <div className="space-y-4">
             <div className="flex justify-end">
                <Button onClick={() => openModal()}><Plus size={16}/> Add Admin</Button>
             </div>
             <Card>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                        {users.map(u => (
                            <tr key={u.id} className="group hover:bg-gray-50 dark:hover:bg-white/5">
                                <td className="py-4 pl-4 font-medium text-gray-900 dark:text-white">{u.username}</td>
                                <td className="py-4"><Badge color="blue">{u.roleName}</Badge></td>
                                <td className="py-4"><Badge color={u.status === 'active' ? 'green' : 'red'}>{u.status}</Badge></td>
                                <td className="py-4 text-sm text-gray-500">{u.lastLogin}</td>
                                <td className="py-4 text-right pr-6 flex justify-end gap-2">
                                    <Button size="sm" variant="ghost" onClick={() => openModal(u)}><Edit2 size={16}/></Button>
                                    <Button size="sm" variant="ghost" className="text-red-500" onClick={() => remove(u.id)}><Trash2 size={16}/></Button>
                                    <Button size="sm" variant="ghost" title="Reset Password"><Key size={16}/></Button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
             ...
             <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Admin User">
                 <div className="space-y-4">
...
const RoleMatrix: React.FC = () => {
    ...
    const PERMISSION_GROUPS = {
        ...
        'Finance': ['finance.read', 'finance.deposit', 'finance.withdraw_audit', 'finance.report'],
        'Product': ['product.read', 'product.create', 'product.edit', 'product.delete'],
        'System': ['system.config', 'admin.manage', 'logs.read']
    };

    const togglePermission = (perm: string) => {
        // Mock implementation for visual feedback
        const has = selectedRole.permissions.includes(perm) || selectedRole.permissions.includes('*.*');
        if (has) {
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 h-[600px]">
             ...
             <div className="w-full md:w-1/4 space-y-2">
                 ...
                 <Button variant="outline" className="w-full mt-4"><Plus size={16}/> New Role</Button>
             </div>
             ...
             <Card className="flex-1 overflow-y-auto">
                 <div className="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-white/5 pb-4">
                     <div>
                         <h3 className="text-lg font-bold text-gray-900 dark:text-white">{selectedRole.name} Permissions</h3>
                         <p className="text-sm text-gray-500">Configure access rights for this role</p>
                     </div>
                 ...
                 <div className="space-y-8">
                     {Object.entries(PERMISSION_GROUPS).map(([group, perms]) => (
                         <div key={group}>
                             <h4 className="font-bold text-gray-700 dark:text-gray-300 mb-3 border-l-4 border-primary-500 pl-3">{group}</h4>
                             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                 {perms.map(perm => {
                                     const isChecked = selectedRole.permissions.includes(perm) || selectedRole.permissions.includes('*.*');
                                     return (
                                         <div 
                                            key={perm} 
                                         ...
                                         >
                                             {isChecked ? <CheckSquare size={18} className="text-green-500" /> : <Square size={18} className="text-gray-400" />}
                                             <span className={`text-sm ${isChecked ? 'font-medium text-green-700 dark:text-green-400' : 'text-gray-500'}`}>{perm}</span>
                                         </div>
                                     );
                                 })}
                             </div>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                    {logs.map(log => (
                        <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-white/5">
                            ...
                            <td className="py-3 font-medium text-gray-900 dark:text-white">{log.adminName}</td>
                            <td className="py-3 text-sm text-gray-800 dark:text-gray-200">{log.action}</td>
                            <td className="py-3 text-xs font-mono text-gray-500">{log.target}</td>
...
export const AdminSecurity: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'admins' | 'roles' | 'logs'>('admins');

    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Admin & Security</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage staff access, roles, and audit trails</p>
            </div>
