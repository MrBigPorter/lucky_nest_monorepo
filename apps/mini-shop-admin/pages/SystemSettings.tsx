...
import { Settings, Image as ImageIcon, Save, Upload, Trash2, PlusCircle } from 'lucide-react';
import { Card, Button, Input, Switch, Select } from '../components/UIComponents';
        ...
        <Card title="Global Configuration">
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
               ...
               <Switch checked={config.maintenanceMode} onChange={c => setConfig({...config, maintenanceMode: c})} />
            </div>

            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-white/5">
               <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Force KYC for Withdrawals</span>
               <Switch checked={config.kycRequired} onChange={c => setConfig({...config, kycRequired: c})} />
            </div>

            <Button className="w-full mt-4"><Save size={18} /> Save Configurations</Button>
          </div>
        </Card>

        {/* Banner Management */}
        <Card title="Banner Management" action={<Button size="sm" variant="outline"><Upload size={14}/> Upload</Button>}>
          <div className="space-y-4">
            {banners.map(banner => (
              <div key={banner.id} className="group relative aspect-video rounded-xl overflow-hidden bg-gray-100 dark:bg-black/20 border border-gray-200 dark:border-white/10">
                <img src={banner.image} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                   <div className="text-white font-bold">{banner.title}</div>
                   <div className="text-xs text-white/80 capitalize">{banner.position} Banner</div>
                   <button 
