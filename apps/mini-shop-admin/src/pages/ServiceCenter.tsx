  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-180px)]">
        {/* List View */}
        <Card
          className={`w-full lg:w-1/3 flex flex-col p-0 overflow-hidden border border-gray-200 dark:border-white/5 ${selectedTicket ? 'hidden lg:flex' : 'flex'}`}
        >
            ...
            {filteredTickets.length === 0 ? (
              ...
              filteredTickets.map((ticket) => (
                ...
                >
                  ...
                  <h4 className="font-medium text-gray-700 dark:text-gray-300 text-xs mb-1 truncate">
                    {ticket.subject}
                  </h4>
        ...
        {/* Chat Detail View */}
        <div
          className={`flex-1 flex flex-col overflow-hidden bg-white dark:bg-dark-800 rounded-xl shadow-sm border border-gray-200 dark:border-white/5 ${selectedTicket ? 'flex' : 'hidden lg:flex'}`}
        >
          {selectedTicket ? (
            <>
              {/* Header */}
              <div className="p-4 bg-white dark:bg-dark-800 border-b border-gray-100 dark:border-white/5 flex justify-between items-center shadow-sm z-10">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="lg:hidden p-2 -ml-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full"
                  >
                    <ChevronLeft size={20} />
                  </button>
                  <img
                  ...
                  />
                  <div>
                    <h2 className="font-bold text-gray-900 dark:text-white text-sm lg:text-base">
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    color={selectedTicket.status === 'open' ? 'red' : 'green'}
                  >
              ...
              <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6 bg-gray-50/50 dark:bg-black/20">
                ...
                {selectedTicket.replies.map((reply, idx) => (
                  <div
                    key={idx}
                    className={`flex gap-3 max-w-[85%] ${reply.sender === 'support' ? 'ml-auto flex-row-reverse' : ''}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 ${reply.sender === 'support' ? 'bg-gradient-to-br from-primary-500 to-primary-600 text-white' : 'bg-gray-200'}`}
                    >
                    ...
                    <div
                      className={`${reply.sender === 'support' ? 'text-right' : ''}`}
                    >
                      <div
                        className={`flex items-baseline gap-2 mb-1 ${reply.sender === 'support' ? 'flex-row-reverse' : ''}`}
                      >
                        <span className="font-bold text-gray-900 dark:text-white text-xs">
                          {reply.sender === 'support'
                            ? 'You'
                      ...
                      <div
                        className={`p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm text-left ${reply.sender === 'support' ? 'bg-primary-500 text-white rounded-tr-none' : 'bg-white dark:bg-dark-800 border border-gray-100 dark:border-white/5 text-gray-700 dark:text-gray-200 rounded-tl-none'}`}
                      >
                        {reply.message}
                      </div>
                    </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 dark:bg-black/20">
              ...
              <p className="text-sm max-w-xs text-center mt-2">
                Choose a ticket from the left sidebar to view details and reply.
              </p>
            </div>
          )}
        </div>
