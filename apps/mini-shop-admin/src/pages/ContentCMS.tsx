...
const ArticleManager: React.FC = () => {
  const {
    ...
    update,
    remove,
  } = useMockData<Article>(MOCK_ARTICLES);
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-white/5">
            {articles.map((a) => (
              ...
              >
                <td className="py-4 pl-4 font-medium text-gray-900 dark:text-white max-w-xs truncate">
                  {a.title}
                </td>
                <td className="py-4">
                ...
                <td className="py-4">
                  <Badge color={a.status === 'published' ? 'green' : 'yellow'}>
                    {a.status}
                  </Badge>
                </td>
                <td className="py-4 text-sm text-gray-500">{a.publishDate}</td>
                <td className="py-4 text-sm text-gray-500 flex items-center gap-1">
                  <Eye size={14} /> {a.views}
                </td>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        {faqs.map((faq) => (
          <div
            key={faq.id}
            className="p-4 bg-white dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5 flex justify-between items-start group hover:border-primary-500/30 transition-all"
          >
            <div>
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="e.g. general, finance"
          />
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="ghost" onClick={() => setIsModalOpen(false)}>
              Cancel
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
          Manage announcements, news, and help center
        </p>
      </div>
