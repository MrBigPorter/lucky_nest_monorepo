...
**当前页面：Operation Logs（操作日志）**

- [ ] 读侧契约定义：新增 `operation-logs` 的 URL 参数解析 / queryKey / request params helper
- [ ] 页面级数据预取：`operation-logs/page.tsx` 增加 Server 预取 + HydrationBoundary 注水
- [ ] 列表消费对齐：`OperationLogsClient` / `OperationLogListClient` 与缓存契约对齐
- [ ] 边界确认：不改 `BaseTable` 全局模型，不改后端接口，只做单页可回退优化
- [ ] 回归验证：`Operation Logs` 相关 Vitest 通过
- [ ] 文档沉淀：新增「Operation Logs 缓存契约」问题与答案 + 心智模型提问

> 下一步：继续按「单页面、可回退」推进下一个 SmartTable 读侧页面。
>
> **参考模板**：`read/features/PRODUCTS_CACHE_CONTRACT_CN.md`、`read/features/USERS_CACHE_CONTRACT_CN.md`、`read/features/ORDERS_CACHE_CONTRACT_CN.md`
